...
class WeightsDownloader:
    ...
    def download(weight_str, url, dest):
        ...
        # For safetensors files, use the -f flag (not -xf) to avoid tar extraction
        is_safetensors = weight_str.endswith('.safetensors')
        if is_safetensors:
            subprocess.check_call(
                ["pget", "--log-level", "warn", "-f", url, os.path.join(dest, os.path.basename(weight_str))],
                close_fds=False
            )
        else:
            # For other files that might be tar archives, use -xf
            subprocess.check_call(
